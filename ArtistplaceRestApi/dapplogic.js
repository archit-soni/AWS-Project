...
export default class DAppObject {
    ...
    constructor() {
        ...
        this.addUserToArtistPlace=this.addUserToArtistPlace.bind(this);
    ...
    getUserProfile(req, res){
        ...
        this.updateRegisteredUserBalance()
        this.registeredUsers.forEach((item)=>{
            if(item['LoginName']==req.params['username']){
                toRet.push(item)
            }
        })
        res.send(toRet)
    ...
    getUserBalance(req, res){
        this.web3obj.eth.getBalance(req.params['address']).then(bal=>{
            res.send(this.web3objobj.utils.fromWei(bal,'ether'));
        })
    }
    //route '/assetsforsale'
    ...
    playMusic(req, res) {
        var sendOption = { from: req.body["fromAccount"], gasPrice: '20000000000', gas: 5000000 };
        this.assetTokenContract.methods.playMusic(req.body["toAccount"], req.body["assetID"],true).send(sendOption).then(() => {
            res.send({ 'status': 'Token Transfered' });
        }).catch(err => {
            res.send({ 'status': 'Error', 'ErrorMsg': err });
    ...
    //route '/adduser'
    addUserToArtistPlace(req, res) {
        // Call DynamoDB to retrieve the list of tables
            this.registeredUsers.push({
                'Name': req.body['Name'],
                'LoginName': req.body['Username'],
                'Bio': req.body['Bio'],
                'EthereumID': req.body['Address'], //the first account will be used by marketplace admin
                'EtherBalance': 0,
                'EscrowBalance': 0
            });
            this.updateRegisteredUserBalance();
